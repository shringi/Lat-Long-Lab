...
import { initImportWizard } from "./modules/import_wizard.js";
...
import "../css/columnControl.css";

// Make globals available for plugins that expect them
window.L = L;
...
function setupEventListeners() {
  ...
  if (tabProcess)
    tabProcess.addEventListener("click", () => switchTab("process"));
  if (getEl("tabExport"))
    getEl("tabExport").addEventListener("click", () => switchTab("export"));
  ...
  if (showTableBtn)
    showTableBtn.addEventListener("click", () => toggleTableVisibility(true));
  // Close Table -> Switch to Full Map View
  if (closeTableBtn)
    closeTableBtn.addEventListener("click", () => switchViewMode("map"));
  ...
  if (confirmMappingBtn)
    confirmMappingBtn.addEventListener("click", () =>
      applyColumnMapping(latColSelect.value, lngColSelect.value),
    );
  if (cancelMappingBtn)
    cancelMappingBtn.addEventListener("click", hideColumnMappingModal);
